...
function fazer_login()
{
    if (isset($_POST['login']) && isset($_POST['senha'])) {
        ...
        $validaLogin = $validaLogin->verifica_login();
        if ($validaLogin == 1) {
            $validaUsuario = new ValidaUsuario();
            $permicao = $validaUsuario->valida_permicao($login);
            $funcionario = $validaUsuario->valida_funcionario($login);
            if ($funcionario != NULL) {
                $dados = $validaUsuario->valida_dados($funcionario);
                $gerarToken = new GeraToken();
                $token = $gerarToken->gerar_token($permicao, $dados);
            } else {
                $gerarToken = new GeraToken();
                $dados['nome'] = NULL;
                $dados['email'] = NULL;
            ...
            $response = array('status_message' => 'O pagamento não foi encontrado',
                'status' => 400);
        } else {
